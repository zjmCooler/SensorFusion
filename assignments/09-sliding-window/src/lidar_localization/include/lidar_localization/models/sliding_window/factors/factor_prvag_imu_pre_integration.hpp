    ...
    virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const {
        ...
        if (jacobians) {
            ...
            if (jacobians[0]) {
                ...
                jacobian_i.setZero();
                // a. residual, position:
                jacobian_i.block<3, 3>(INDEX_P, INDEX_P) = -R_i_inv; // drp_dpi
                jacobian_i.block<3, 3>(INDEX_P, INDEX_R) = Sophus::SO3d::hat(
                    R_i_inv * (pos_j - pos_i - vel_i * T_ + 0.5 * g_ * T_ * T_)); // drp_dri
                jacobian_i.block<3, 3>(INDEX_P, INDEX_V) = - T_ * R_i_inv; // drp_dvi
                ...
                jacobian_i.block<3, 3>(INDEX_R, INDEX_G) = -J_r_inv *
                    Sophus::SO3d::exp(residual.block<3, 1>(INDEX_R, 0)).inverse().matrix() *
                    J_.block<3, 3>(INDEX_R, INDEX_G);

                // c. residual, velocity:
                jacobian_i.block<3, 3>(INDEX_V, INDEX_R) = Sophus::SO3d::hat(R_i_inv * (vel_j - vel_i + g_ * T_));
