...
ErrorStateKalmanFilter::ErrorStateKalmanFilter(const YAML::Node &node) {
    ...
    COV.PROCESS.GYRO_RANDOM_WALK = node["covariance"]["process"]["gyro_random_walk"].as<double>();
    COV.PROCESS.ACCEL_RANDOM_WALK = node["covariance"]["process"]["accel_random_walk"].as<double>();
    ...
    LOG(INFO) << std::endl
              ...
              << "\tprocess noise gyro random walk.: " << COV.PROCESS.GYRO_RANDOM_WALK << std::endl
              << "\tprocess noise accel random walk.: " << COV.PROCESS.ACCEL_RANDOM_WALK << std::endl
              << std::endl
    ...
    Q_.block<3, 3>(6, 6) = COV.PROCESS.ACCEL_RANDOM_WALK * Eigen::Matrix3d::Identity();
    Q_.block<3, 3>(9, 9) = COV.PROCESS.GYRO_RANDOM_WALK * Eigen::Matrix3d::Identity();

    NoiseVec_.block<3, 1>(0, 0) = COV.PROCESS.ACCEL * Eigen::Vector3d::Ones();
    NoiseVec_.block<3, 1>(3, 0) = COV.PROCESS.GYRO * Eigen::Vector3d::Ones();
    NoiseVec_.block<3, 1>(6, 0) = COV.PROCESS.ACCEL_RANDOM_WALK * Eigen::Vector3d::Ones();
    NoiseVec_.block<3, 1>(9, 0) = COV.PROCESS.GYRO_RANDOM_WALK * Eigen::Vector3d::Ones();
    ...
    RPoseVelCons_.block<3, 3>(3, 3) = COV.MEASUREMENT.POSE.ORI * Eigen::Matrix3d::Identity();
    RPoseVelCons_.block<2, 2>(6, 6) = COV.MEASUREMENT.VEL * Eigen::Matrix2d::Identity();  //初始化观测噪声
    ...
    GPoseVelCons_.block<3, 3>(3, INDEX_ERROR_ORI) = Eigen::Matrix3d::Identity();
    CPoseVelCons_.block<8, 8>(0, 0) = Eigen::Matrix<double, 8, 8>::Identity();
...
void ErrorStateKalmanFilter::Init(
    ...
    pose_.block<3, 3>(0, 0) = C_nb; // N系的位姿
...
//    linear_acc_init = linear_acc_init - accl_bias_; // v_b
//    angular_vel_init = angular_vel_init - gyro_bias_; // w_b
...
bool ErrorStateKalmanFilter::GetAngularDelta(
    ...
    angular_vel_mid = 0.5 * (angular_vel_prev + angular_vel_curr);
    return true;
}
...
void ErrorStateKalmanFilter::SetProcessEquation(
    ...
    B_.block<9, 9>(INDEX_ERROR_ORI, INDEX_ERROR_VEL) = Eigen::Matrix<double, 9, 9>::Identity(); //考虑随机游走
//    B_.block<3, 3>(INDEX_ERROR_ORI, INDEX_ERROR_VEL) = Eigen::Matrix<double, 3, 3>::Identity(); //不考虑随机游走
}
...
void ErrorStateKalmanFilter::UpdateErrorEstimation(
    ...
    MatrixB B = T * B_;
    B.block<6, 6>(9, 6) = Eigen::Matrix<double, 6, 6>::Identity() * sqrt(T); //考虑随机游走

    // 
    // TODO: perform Kalman prediction
    //
//    X_ = F * X_ + B * NoiseVec_; // fix this
    X_ = F * X_;
...
void ErrorStateKalmanFilter::CorrectErrorEstimationPose(
    ...
    // 
    Eigen::Vector3d P_nn_obs = pose_.block<3, 1>(0, 3) - T_nb.block<3, 1>(0, 3);  // pose - p_measure    // fix this
    ...
    Eigen::MatrixXd tmp = G * P_ * G.transpose() + CPose_ * R * CPose_.transpose();
    K = P_ * G.transpose() * tmp.inverse(); // fix this
//    LOG(INFO) << K << std::endl;
}

void ErrorStateKalmanFilter::CorrectErrorEstimationPoseVelCons(const Eigen::Matrix4d &T_nb,
    ...
    Eigen::Vector3d P_nn_obs = pose_.block<3, 1>(0, 3) - T_nb.block<3, 1>(0, 3);  // pose - p_measure    // fix this
    Eigen::Matrix3d C_nn_obs = T_nb.block<3, 3>(0, 0).transpose() * pose_.block<3, 3>(0, 0); // fix this
    Eigen::Vector3d v_bb_obs = pose_.block<3, 3>(0, 0).transpose() * vel_ - v_b; // delta_v
...
//    YPose_.block<3, 1>(3, 0) = Sophus::SO3d::vee(C_nn_obs - Eigen::Matrix3d::Identity());
    ...
    YPoseVelCons_.block<2, 1>(6, 0) = v_bb_obs.block<2, 1>(1, 0);  //v_yz

    Y = YPoseVelCons_;  // 8 * 1

    // set measurement equation:
    Eigen::Matrix3d R_bn = pose_.block<3, 3>(0, 0).transpose();
    GPoseVelCons_.block<2, 3>(6, 3) = R_bn.block<2, 3>(1, 0);
    GPoseVelCons_.block<2, 3>(6, 6) = Sophus::SO3d::hat(v_bb_obs).matrix().block<2, 3>(1, 0);
    G = GPoseVelCons_; // 8 * 15

    //
    // TODO: set Kalman gain:
    //
    MatrixRPoseVelCons R = RPoseVelCons_; // 8 * 8
    Eigen::MatrixXd tmp = G * P_ * G.transpose() + CPoseVelCons_ * R * CPoseVelCons_.transpose(); // 8 * 8
    K = P_ * G.transpose() * tmp.inverse(); // fix this 15 * 8
...
void ErrorStateKalmanFilter::CorrectErrorEstimation(
    ...
    switch (measurement_type) {
        ...
        case MeasurementType::POSE_VEL:
            CorrectErrorEstimationPoseVelCons(measurement.T_nb, measurement.v_b, measurement.w_b,
                                              Y, G, K);
            break;
        default:break;
    }
    ...
    //
    P_ = (Eigen::Matrix<double, DIM_STATE, DIM_STATE>::Identity() - K * G) * P_; // fix this
...
void ErrorStateKalmanFilter::EliminateError(void) {
    ...
    //
    // a. position:
    pose_.block<3, 1>(0, 3) = pose_.block<3, 1>(0, 3) - X_.block<3, 1>(INDEX_ERROR_POS, 0); // fix this
    // b. velocity:
    vel_ = vel_ - X_.block<3, 1>(INDEX_ERROR_VEL, 0);                                       // fix this
    // c. orientation:
    pose_.block<3, 3>(0, 0) = pose_.block<3, 3>(0, 0) *
        (Eigen::Matrix3d::Identity() - Sophus::SO3d::hat(X_.block<3, 1>(INDEX_ERROR_ORI, 0)).matrix()); // fix this
...
void ErrorStateKalmanFilter::UpdateObservabilityAnalysis(
    ...
    Eigen::VectorXd Y;
    switch (measurement_type) {
        case MeasurementType::POSE:GetQPose(Q, Y);
            break;
        case MeasurementType::POSE_VEL:GetQPoseVel(Q, Y);
            break;
        case MeasurementType::POSI:GetQPosi(Q, Y);
            break;
        case MeasurementType::POSI_VEL:GetQPosiVel(Q, Y);
            break;
        default:break;
    }
...
bool ErrorStateKalmanFilter::SaveObservabilityAnalysis(
    ...
    std::string type;
    switch (measurement_type) {
        case MeasurementType::POSE:type = std::string("pose");
            break;
        case MeasurementType::POSE_VEL:type = std::string("pose_velocity");
            break;
        case MeasurementType::POSI:type = std::string("position");
            break;
        case MeasurementType::POSI_VEL:type = std::string("position_velocity");
            break;
